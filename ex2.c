...
// Function input: source file, target file, source flag, target flag and int if need to swap bytes.
// Function Action: change the end line from mac to unix or unix to mac.
// Function output: none
// **************************************************************************************************/
void changeEndLineOfMacAndUnix(char* in, char* out, enum endLinePerSystem flag1, enum endLinePerSystem flag2, int needToChange) {
  FILE* source = fopen(in, "rb");
  if (source != NULL) {
    FILE *target = fopen(out, "wb");
    if (target != NULL) {
...
// Function Action: change end line of source file from windows to mac and unix, and from unix and mac to windows.
//                  if the argc[5] is KEEP
// Function output: none
// **************************************************************************************************/

void endLineOfWindows(char* in, char* out, enum endLinePerSystem flag1, enum endLinePerSystem flag2) {
  FILE* source = fopen(in, "rb");
  if (source != NULL) {
    FILE *target;
    target = fopen(out, "wb");
...
// Function output:
// **************************************************************************************************/
void changeendLineOfWindows(char* in, char* out, enum endLinePerSystem flag1, enum endLinePerSystem flag2) {
  FILE* source = fopen(in, "rb");
  if (source == NULL) {
    return;
  }
...
// Function input: char
// Function Action: if the char is '-swap' return 1 if the char is '-keep' return 0.
// Function output:
// **************************************************************************************************/
